class PhotosController < ApplicationController
  ...
  before_filter only: [:destroy, :update, :create, :edit, :newteam, :newathlete, :untagathlete, :untagteam, :update, 
                      :untag_athletes, :tag_athletes, :createmobile] do |controller| 
#    controller.SiteOwner?(@photo.nil? ? nil : @photo.team_id)
    if sportUserPics?(@sport) or SiteAdmin?(@sport)
      true
    else
      false
    end
